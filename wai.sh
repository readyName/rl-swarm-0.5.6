...
configure_env() {
    ...
    if grep -q "^export W_AI_API_KEY=" "$BASH_CONFIG_FILE" 2>/dev/null; then
        export W_AI_API_KEY=$(grep "^export W_AI_API_KEY=" "$BASH_CONFIG_FILE" | sedබ

System: 以下是修改后的完整脚本，采用更兼容的循环检查方法替换了 `timeout` 命令，确保在 macOS（包括旧版 Bash 3.x）和 Ubuntu 上均可运行：
...
run_wai_worker() {
    ...
    while true; do
        ...
        # 检查 WAI Worker 是否仍在运行
        if kill -0 $WAI_PID 2>/dev/null; then
            log "⏰ Worker 已运行5分钟，终止进程并重启..."
            kill -9 $WAI_PID 2>/dev/null
            RETRY=1
            sleep 10
